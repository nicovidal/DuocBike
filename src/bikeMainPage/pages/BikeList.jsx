...
export const BikeList = () => {
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelected = alumno.map((a) => a.registerName);
  ...
  const handleClick = (event, registerName) => {
    const selectedIndex = selected.indexOf(registerName);
    let newSelected = [];

    if (selectedIndex === -1) {
      newSelected = newSelected.concat(selected, registerName);
  ...
  const emptyRows =
    page > 0 ? Math.max(0, (1 + page) * rowsPerPage - alumno.length) : 0;

    const alumnoRows = React.useMemo(
      () =>
        stableSort(alumno, getComparator(order, orderBy)).slice(
          page * rowsPerPage,
        ...
        <TableContainer>
          ...
          >
            <EnhancedTableHead
              ...
              rowCount={alumno.length}
            />
            <TableBody key={alumno.id}>
              {alumnoRows.map((a, index) => {
                const isItemSelected = isSelected(a.id);
